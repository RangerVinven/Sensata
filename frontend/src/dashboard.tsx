...
function Dashboard() {
  ...
  async function getSensors() {
    try {
      const response = await fetch(
        "https://idp_api.arfff.dog/api/v1/all_sensors",
        { credentials: "include" },
      );
  ...
  async function getSensorData(sensor_id: number) {
    try {
      const response = await fetch(
        `https://idp_api.arfff.dog/api/v1/sensor_data/${sensor_id}`,
      );
  ...
  function getTodaysTraffic(sensor_data: Array<SensorData>) {
    const todaysTrafficArray = sensor_data.filter((data) => {
      ...
      return (
        time_recorded.getDate() === today.getDate() &&
        time_recorded.getMonth() === today.getMonth() &&
        time_recorded.getFullYear() === today.getFullYear()
      );
    });
  ...
  function getDailyAverageTraffic(traffic: Array<SensorData>) {
    ...
    average =
      average == 0
        ? 0
        : Math.round(average / Object.keys(trafficPerDay).length);
  ...
  async function loadDashboard(sensor_index: number = 0) {
    ...
    const sensor_data = await getSensorData(
      sensors_json?.[sensor_index].sensor_id,
    );
        ...
        <div id="Charts-Select-Container">
          ...
          <div id="All-Charts-Container">
            {isLoading ? (
              ...
              <h3 id="Loading-Icon">Sensor has no data</h3>
            ) : (
              <>
                <div className="Chart-Container">
                  <LineChartComponent
                    pastSevenDaysTraffic={pastSevenDaysTraffic}
                  />
                </div>
                <div className="Chart-Container">
